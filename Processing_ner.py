...
def build_csv(excel_path, word_path, save_path):  
    ...
    """
    dfall = None
    for info in os.listdir(path_csv): 
        if info.endswith(".xlsx"):             
            domain = os.path.abspath(path_csv) #获取文件夹的路径 
            info = os.path.join(domain,info) #将路径与文件名结合起来就是每个文件的完整路径 
            df = pd.read_excel(info) 
            dfall= pd.concat([dfall,df],axis=0,ignore_index=True) #拼接dataframe
    dfall=res.dropna(axis=0) #去掉有空字段的数据
    dfall = dfall.drop_duplicates()#去重
    ...
    dfall.to_csv(os.path.join(path_csv,"df.csv"), index=False, encoding='utf_8_sig')


    df = pd.read_csv(os.path.join(path_csv,"df.csv"), encoding='utf-8')
...
def built_ner_data(path_ner,save_path):
    ...
    for index,row in dfall.iterrows():   #遍历dataframe的每一行
        a = ''
        if data1.loc[index,"案由"] == "申请执行人执行异议之诉":
            data1.loc[index,"案由"] =  "申请执行人执行异议之与"
...
if __name__ == '__main__':
    ...
    build_csv("D:/corpus/test", 'D:/corpus/test/无讼案例批量下载_2019_7_19_15_2_47_942', "D:/" )
    built_ner_data("D:/df.csv","D:/df_ner.txt")