...
dfall = None
for info in os.listdir('D:/corpus/test'): 
    if info.endswith(".xlsx"):             
        domain = os.path.abspath('D:/corpus/test') #获取文件夹的路径 
...
dfall.to_csv("D:/test.csv", index=False, encoding='utf_8_sig')
...
for index,row in dfall.iterrows():   #遍历dataframe的每一行
    ...
    str2ner_train_data(a,"D:/ner.txt")
